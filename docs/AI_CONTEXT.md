...
### Phase 7: 利便性向上（完了・本番稼働中）
## 最終更新: 2026-05-14

- [x] **タスク複製機能**: 既存タスクを元に新しいタスクをワンクリックで作成
  - `TaskCard` に「📋 コピーして作成」ボタン追加（親のみ・全ステータス対象）
  - `TaskForm` に `initialData` Props 追加（タイトル・説明・ポイント・担当者を自動注入）
  - ボタン押下時にフォームへ自動スクロール
  - DB書き込みはフォームの「タスクを投稿」ボタン押下時のみ（誤操作防止）
